# modified version of https://github.com/colour-science/colour/blob/v0.4.2/colour/notation/munsell.py
...
def _round_value(value: FloatingOrArrayLike) -> FloatingOrNDArray:
    if value > 1.0:
        value = round(value)
    else:
...
def _munsell_specification_to_xyY(specification: ArrayLike) -> NDArray:
    ...
    if value >= 1.0:
        ...
        if is_integer(value * 10.0):
            value_minus = value_plus = round(value * 10.0) / 10.0
        else:
            value_minus = np.floor(value * 10.0) / 10.0
            value_plus = value_minus + 0.1
...
def munsell_specification_to_munsell_colour(
    ...
    if is_grey_munsell_colour([hue, value, chroma, code]):
        ...
        attest(
            ...
            f"domain [0, 10]! (normalized as [{hue}, {value}, {chroma}, {code}])",
        )
...
def interpolation_method_from_renotation_ovoid(
    ...
    if is_grey_munsell_colour(specification):
        ...
        if value > 1.0:
            ...
            attest(
                is_integer(value * 10.0),
                f'"{specification}" specification value * 10.0 must be an integer!',
            )
...
def maximum_chroma_from_renotation(
    ...
    attest(
        0 < value <= 10,
        f'"{value}" value must be normalised to domain (0, 10]!',
    )
